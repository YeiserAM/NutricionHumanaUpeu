...
export class ReportAdminComponent implements OnInit {
  constructor(private router: Router, private reportService: ReportesService) {}
  ...
  reportGraphicBarras() {
    this.reportService.getReportsAdminMensual().subscribe(res => {
      new Chart("myChart", {
        type: "bar",
        data: {
          labels: res.dataMes,
  ...
  reportGraphicTorta() {
    this.reportService.getReportsAdmin().subscribe((resp: any) => {
      new Chart("graphycTorta", {
        type: "doughnut",
        data: {
          labels: ["Rechazado", "Pendientes", "Observados", "Aceptados"],
          datasets: [
            {
